...
const StripePaymentSubs = ({
  ...
  const setPaymentMethodOriginalBeforeThePlane = async (method: string) => {
    ...
    if (isBensPaymentProvider(method)) {
      const start = await initiatePaymentSession(cart, {
        provider_id: method,
      });
  ...
  const getSkus = async() => {
    let skus = [];
      if (Array.isArray(cart.items)) {
        skus = cart.items.map(item => ({price : item.variant_sku}));
      }
      return skus
    }
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      const shouldInputCard = isBensPaymentProvider(selectedPaymentMethod) && !activeSession;
      if (!stripe || !elements) {
      ...
      if (!checkActiveSession) {
        const customerDetails = {
          ...
          if (error) { // Handle any errors returned by Stripe
            console.error('Error creating payment method:', error.message);
          } else {
            console.log('Payment Method ID:', paymentMethod.id);
            //  send this payment method ID to your backend server
          }
...
export default StripePaymentSubs;
