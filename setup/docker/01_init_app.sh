...
# Prefer APP_* if set; fall back to official envs or sane defaults
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
  -v db_user="$APP_DB_USER" -v db_pass="$APP_DB_PASS" <<'SQL'
SELECT
  CASE
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
  -v db_name="$APP_DB_NAME" -v db_user="$APP_DB_USER" <<'SQL'
...
# Import dump if present
DUMP="/docker-entrypoint-initdb.d/import/mydb_clean_export.sql"
if [ -f "$DUMP" ]; then
  echo "Importing dump into $APP_DB_NAME ..."
  psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$APP_DB_NAME" -f "$DUMP"
